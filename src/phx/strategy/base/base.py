...
class StrategyBase(StrategyInterface, abc.ABC):
    ...
    def round(self, price: float, direction: RoundingDirection, ticker: Ticker, min_tick_size=None) -> Optional[float]:
        min_tick_size = min_tick_size if min_tick_size is not None else self.security_list.get(ticker, 0)
    ...
    def round_down(self, price: float, key: Ticker, min_tick_size=None) -> Optional[float]:
        return self.round(price, RoundingDirection.DOWN, key, min_tick_size)

    def round_up(self, price: float, key: Ticker, min_tick_size=None) -> Optional[float]:
        return self.round(price, RoundingDirection.UP, key, min_tick_size)
